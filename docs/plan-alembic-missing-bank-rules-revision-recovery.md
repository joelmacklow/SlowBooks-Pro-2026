...
## Objective
Restore startup compatibility for environments whose database `alembic_version` points at the removed Bank Rules MVP revision `o5d6e7f8g9h0`, without reintroducing the whole unmerged Bank Rules feature onto the current branch.
...
- The current branch does not ship that migration file.
...
## Constraints
- Keep branch behavior aligned with the currently shipped schema/features.
- Do not resurrect the full Bank Rules feature just to satisfy migration lookup.
- Preserve a single linear migration chain.
- Recovery must be safe for both:
...
- Any leftover bank-rules tables in previously upgraded environments remain harmless because this branch does not reference them.
...
## Acceptance criteria
- Alembic can resolve revision `o5d6e7f8g9h0` on this branch.
- Migration integrity test still reports one linear chain and one head.
- Fresh upgrades from `n4c5d6e7f8g9` remain valid.
- No current branch schema tables/columns are added by this recovery slice.
...
- **Risk:** Future work assumes `o5d6e7f8g9h0` carried Bank Rules schema.
  - **Mitigation:** Document clearly in the migration file that this is a compatibility placeholder/no-op.
- **Risk:** Existing databases that already have old bank-rules tables drift from code expectations.
  - **Mitigation:** Current branch ignores those tables, which is acceptable until/unless the feature is reintroduced properly.
