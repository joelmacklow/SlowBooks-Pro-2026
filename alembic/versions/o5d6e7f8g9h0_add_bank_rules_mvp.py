...
This revision intentionally acts as a no-op.

The original Bank Rules MVP branch introduced this revision id before that
feature branch was closed without merging to the current branch. Some
environments were already stamped with `o5d6e7f8g9h0`, which means Alembic must
still be able to resolve that revision during bootstrap.

Keeping this compatibility placeholder restores a linear migration chain
without reintroducing the unmerged Bank Rules schema.
"""
...
def upgrade() -> None:
    # Compatibility no-op: intentionally left blank.
    pass
